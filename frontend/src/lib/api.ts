...
// docs/adr/ADR-0005 - a ClaudeAccount's Token never round-trips over the wire (the
// backend's Token field is [JsonIgnore]'d), so there's nothing here shaped like
// Project.hasGitCredential to check - the account existing at all IS the signal.
...
interface ClaudeAccountUsageWindow {
  windowStart: string
  costUsd: number
  promptTokens: number
  completionTokens: number
...
export interface ClaudeAccountUsage extends ClaudeAccountSummary {
  // Forge's own estimate from its own Run history (rolling 5h / rolling 7d) - not a
  // real queried quota, Anthropic exposes no API for that. Does not reflect live
  // cooldown state either - that's in-memory inside the separate Worker process.
  sessionUsage: ClaudeAccountUsageWindow
  weeklyUsage: ClaudeAccountUsageWindow
}
...
export const api = {
  ...
  // docs/adr/ADR-0005 - multi-account Claude failover. `token` comes from
  // `claude setup-token`, run by hand once per account - never generated by Forge.
  // Never round-trips back (the backend's Token is [JsonIgnore]'d), so there's no
  // "current value" to show or edit - only add/rename/toggle/remove.
  listClaudeAccounts: (userId: string) => request<ClaudeAccountUsage[]>(`/users/${userId}/claude-accounts`),
  addClaudeAccount: (userId: string, name: string, token: string) =>
    request<ClaudeAccountSummary>(`/users/${userId}/claude-accounts`, {
      method: 'POST',
      body: JSON.stringify({ name, token }),
    }),
  updateClaudeAccount: (accountId: string, patch: { name?: string; isActive?: boolean }) =>
    request<ClaudeAccountSummary>(`/claude-accounts/${accountId}`, {
      method: 'PATCH',
      body: JSON.stringify(patch),
    }),
