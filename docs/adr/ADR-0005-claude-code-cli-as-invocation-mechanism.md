...
- **Multi-account fallback: built (2026-08-08), founder-requested** — "o recurso de poder cadastrar multiplas contas do claude e ele reautenticar automaticamente para nunca parar o trabalho esta se tornando cada vez mais necessario." `ClaudeAccountPool.cs` holds the list of configured `CLAUDE_CONFIG_DIR` paths (env var `CLAUDE_ACCOUNT_CONFIG_DIRS`, colon-separated) and an in-memory per-account cooldown; `ClaudeCliProvider.InvokeAsync` (`ClaudeCliProvider.cs`) tries each available (not-cooling-down) account in order, and on a detected usage-limit failure (`ClaudeUsageLimitException`) marks that account cooling down and rotates to the next before giving up. Verified live against a fake `claude` binary standing in for a real second account (no second real account exists yet to test against) — confirmed a rotation on the first call, and the exhausted account correctly skipped without a wasted attempt on the second.
  - **Still genuinely blocked on one thing only the founder can do**: each additional account needs its own interactive OAuth device/browser login completed once — `CLAUDE_CONFIG_DIR=/some/new/path claude` in a terminal, then `/login` — before its path is worth adding to `CLAUDE_ACCOUNT_CONFIG_DIRS`. The mechanism is ready the moment a second logged-in path exists; nothing else needs to change.
  ...
  - **Known limitation, accepted for now**: cooldown state is in-memory, per-Worker-process — lost on a restart, and would need moving to Postgres if this ever ran as more than one Worker process (it doesn't today, [[007-ExecutionEngine]] §1).
