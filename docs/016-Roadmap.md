...
- [x] **Planner agent — real implementation.** Calls the Claude Code CLI directly per [[ADR-0005]] (not the Anthropic API — that blocker turned out to be avoidable). Reads `Project.LocalPath`'s real checkout, produces a description + acceptance criteria or genuine clarifying questions, records real cost/token usage on `Run`. Validated live against `felippedomingos/forge-test-sandbox` — see the commit implementing it for the full transcript. Web/issue-tracker MCP tool access ([[009-MCP]]) still not wired — today it only reads the local checkout.
- [ ] **Developer agent — real implementation.** Currently a stub. Needs: worktree sync/create/reuse ([[007-ExecutionEngine]] §2), the actual agent loop (now unblocked — can reuse `ClaudeCliProvider` the same way Planner does), build/test execution, live trace streaming.
