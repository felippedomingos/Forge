...
## 6. Per-Project Git Credential (founder-requested, 2026-08-07)

Found live the same day: `GitOps.RunAsync`'s `git fetch`/`push` relied entirely on whatever credential already happened to be configured on the host (an embedded PAT in the on-disk remote URL, or SSH keys), and `RunGhAsync`/`RunAzAsync` inherited a single Worker-process-wide `GH_TOKEN`/`AZURE_DEVOPS_EXT_PAT` env var shared by every project — Forge itself had no knowledge of or control over git auth at all. When ActizOps's embedded PAT expired, `git fetch` failed with no visibility into why until diagnosed by hand (see [[006-Scheduler]]/[[005-Agents]] §4's same-day fix for the crash-loop that caused).

**`Project.GitCredential`** (nullable string, plaintext in Postgres — no encryption-at-rest infra exists yet, a deliberate scope decision matching this project's current security posture, not an oversight) is now the source of truth Forge actively uses, not a passive record:

- `GitOps.EnsureAuthenticatedRemoteAsync` rewrites the `origin` remote's URL from `Project.RepositoryUrl` (the clean, credential-free value) plus the stored credential immediately before `git fetch` (`AgentActivities.DevelopAsync`) and `git push` (`GitFinalizeAsync`) — a no-op when `GitCredential` is null (falls back to whatever's already configured, unchanged behavior) or when `RepositoryUrl` isn't `https://` (an SSH remote authenticates via host-configured keys). `x-access-token` is used as a placeholder username for both providers — GitHub's own documented convention, and Azure DevOps accepts any non-empty username with a PAT as the password.
- `GitOps.RunGhAsync`/`RunAzAsync` accept the credential as a parameter and set `GH_TOKEN`/`AZURE_DEVOPS_EXT_PAT` on that one subprocess call only, overriding the Worker's ambient env var without mutating it — so different projects on the same Worker can use different credentials for PR creation/status checks.

**Never exposed over HTTP in plaintext**: `Project.GitCredential` is `[JsonIgnore]`'d — every endpoint that returns a `Project` returns the tracked entity directly (no response DTO), so this is the only thing stopping it from serializing into a `GET`/`PATCH` response. `Project.HasGitCredential` (`[NotMapped]`, computed) is what the frontend actually receives — enough to show "PAT configured" without ever seeing or re-transmitting the real value. `PATCH /projects/{id}`'s `gitCredential` field is therefore write-only and tri-state, unlike every other field on that endpoint: omitted/null leaves the stored value untouched, an explicit empty string clears it, anything else sets/rotates it — the frontend has no way to "leave it as-is by sending back what's there" since it never has the real value to send.
